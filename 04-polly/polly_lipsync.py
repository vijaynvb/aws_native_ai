...
# Amazon Polly's "pcm" output format is signed 16-bit little-endian, mono, 16 kHz.
POLLY_PCM_SAMPLE_WIDTH = 2
POLLY_PCM_CHANNELS = 1
POLLY_PCM_SAMPLE_RATE = 16000


def pcm_to_wav_bytes(pcm_bytes):
    """
    Wraps raw PCM audio data, as returned by Amazon Polly, in an in-memory WAV
    file so it can be played with the winsound module.

    :param pcm_bytes: The raw PCM audio bytes returned by Amazon Polly.
    :return: The bytes of a WAV file containing the audio data.
    """
    wav_buffer = io.BytesIO()
    with wave.open(wav_buffer, "wb") as wav_file:
        wav_file.setnchannels(POLLY_PCM_CHANNELS)
        wav_file.setsampwidth(POLLY_PCM_SAMPLE_WIDTH)
        wav_file.setframerate(POLLY_PCM_SAMPLE_RATE)
...
class PollyMouth:
    ...
    def say_it(self):
        ...
        if audio_stream is not None:
            wav_bytes = pcm_to_wav_bytes(audio_stream.read())
            fd, speech_file_name = tempfile.mkstemp(suffix=".wav")
            with os.fdopen(fd, "wb") as speech_file:
                speech_file.write(wav_bytes)
            winsound.PlaySound(
                speech_file_name, winsound.SND_FILENAME | winsound.SND_ASYNC
            )
            # winsound plays asynchronously, so the file can't be deleted right
            # away. Estimate playback duration from the WAV size and clean up
            # after it has had time to finish.
            duration_ms = int(
                len(wav_bytes) / (POLLY_PCM_SAMPLE_WIDTH * POLLY_PCM_SAMPLE_RATE) * 1000
            )
            self.app.after(
                duration_ms + 1000,
                lambda: os.path.exists(speech_file_name)
                and os.remove(speech_file_name),
            )
